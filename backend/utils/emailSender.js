...
const sendEmail = async (to, subject, html, attachments = []) => {
  try {
    const info = await transporter.sendMail({
      from: `"ATPL Solar EPC" <${process.env.EMAIL_USER}>`,
      to,
      subject,
      html,
      attachments
...
const sendInvoiceEmail = async (to, invoiceData, pdfPath) => {
  const html = `
    <h2>Invoice from ATPL Solar EPC</h2>
    <p>Dear Client,</p>
    ...
    <br>
    <p>Best regards,<br>ATPL Solar EPC Team</p>
  `;

  return await sendEmail(to, `Invoice ${invoiceData.invoiceNumber} - ATPL Solar EPC`, html, [
